...
use halo2curves_axiom::extend_field_legendre;
use rand::RngCore;
...
impl halo2curves_axiom::ff::Field for Fq {
    ...
    fn sqrt(&self) -> CtOption<Self> {
        ...
        halo2curves_axiom::secp256r1::Fq::from_bytes(&self.to_bytes()).and_then(|v| {
            halo2curves_axiom::ff::helpers::sqrt_tonelli_shanks(&v, tm1d2)
                .and_then(|v| Self::from_bytes(&v.to_bytes()))
        })
    }

    fn sqrt_ratio(num: &Self, div: &Self) -> (Choice, Self) {
        let num = halo2curves_axiom::secp256r1::Fq::from_bytes(&num.to_bytes()).unwrap();
        let div = halo2curves_axiom::secp256r1::Fq::from_bytes(&div.to_bytes()).unwrap();

        let (choice, ret) = halo2curves_axiom::ff::helpers::sqrt_ratio_generic(&num, &div);
        (choice, Self::from_bytes(&ret.to_bytes()).unwrap())
    }
}
